...
def run_test():
    ...
    print("[TEST] Uploading file to Node 8000...")
    headers = {"X-MeshCloud-Token": "meshcloud_secret_token"}
    with open(test_file, "rb") as f:
        resp = requests.post(
            "http://localhost:8000/upload", files={"file": f}, headers=headers
        )
    ...
    for attempt in range(max_attempts):
        ...
        for port in range(START_PORT, START_PORT + NUM_NODES):
            try:
                ...
                r = requests.get(
                    f"http://localhost:{port}/has_file/{file_hash}",
                    headers=headers,
                    timeout=2,
                )
        ...
        print(
            f"  [Attempt {attempt+1}/{max_attempts}] Found on {found_on_count} nodes: {nodes_with_file}"
        )
    ...
    print(
        f"[FAIL] Replication failed! Only found on {found_on_count} nodes, expected at least {REPLICATION_FACTOR}."
    )

    # Print some logs from node 8000 if it failed
    print("\n--- NODE 8000 LOG (Last 50 lines) ---")
    os.system("tail -n 50 /tmp/node8000.log")
    print("---------------------\n")
    return False
