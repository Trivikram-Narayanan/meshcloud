...
if [ ! -f "$REPO/.env" ]; then
  ...
  warn "⚠️  NEXT STEP: Generate and set a strong ADMIN_PASSWORD"
  warn "  Run: export ADMIN_PASSWORD=\"$(python3 -c 'import secrets; print(secrets.token_urlsafe(16))')\" "
  warn "  Then add to .env: ADMIN_PASSWORD=$ADMIN_PASSWORD"
else
