    ...
    async with lock:
        if chunk_path.exists():
            ...
            await loop.run_in_executor(
                None, add_uploaded_chunk, upload_id, chunk_index, chunk_hash
            )
            return
    ...
    try:
        ...
        with os.fdopen(tmp_encrypted_fd, "wb") as f_out:
            ...
            for chunk_hash in chunks:
                ...
                if not chunk_path.exists():
                    raise HTTPException(
                        status_code=400, detail=f"Missing chunk: {chunk_hash}"
                    )
        ...
        logger.exception(
            f"Failed to finalize upload for {filename} (session={upload_id})"
        )
        raise HTTPException(
            status_code=500, detail="Upload finalization failed"
        ) from exc
    finally:
...
async def handle_legacy_upload(file: UploadFile, is_replica: bool = False, owner: str | None = None) -> dict:
    ...
    try:
        ...
        logger.info(
            f"Legacy upload stored: {file.filename} → {file_hash} (is_replica={is_replica})"
        )
...
async def stream_file(file_hash: str) -> AsyncGenerator[bytes, None]:
    ...
    if chunk_hashes:
        ...
        for chunk_hash in chunk_hashes:
            ...
            if not chunk_path.exists():
                raise HTTPException(
                    status_code=500, detail=f"Missing chunk: {chunk_hash}"
                )
            ...
            if chunk_hasher.hexdigest() != chunk_hash:
                ...
                raise HTTPException(
                    status_code=409, detail=f"Integrity failure in chunk {chunk_hash}"
                )
        return
