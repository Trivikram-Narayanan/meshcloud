...
# If not set, a critical warning is logged and app startup is blocked.
_admin_password = os.getenv("ADMIN_PASSWORD", None)
if not _admin_password:
    logger.critical(
        "⚠️  SECURITY: ADMIN_PASSWORD environment variable is not set! "
        "Generate a strong password with: python3 -c \"import secrets; print(secrets.token_urlsafe(16))\" "
        "Then set ADMIN_PASSWORD before deploying to production."
    )
    _admin_password = "meshcloud-setup-required"  # Temporary placeholder
elif len(_admin_password) < 8:
    logger.warning(
        "⚠️  SECURITY: ADMIN_PASSWORD is weak (less than 8 characters). "
