...
def replication_monitor_worker():
    ...
    while True:
        try:
            ...
            under = [
                f for f in files if len(get_file_locations(f.hash)) < REPLICATION_FACTOR
            ]
            if under:
                logger.warning(
...
def sync_with_peer(peer, file_path, filename, file_hash):
    ...
    if chunks:
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [
                executor.submit(replicate_single_chunk, chunk) for chunk in chunks
            ]
        ...
        if not file_p.exists():
            logger.warning(
                f"No chunks and no local file for {file_hash}, cannot replicate"
            )
            return
...
def propagate_to_peers(file_path, filename, file_hash):
    ...
    if needed <= 0:
        logger.debug(
            f"Replication target met for {file_hash} ({len(current_locations)} copies)"
        )
        return
    ...
    if not potential_peers:
        logger.warning(
            f"No potential peers found to replicate {file_hash}. Needed: {needed}"
        )
        return
    ...
    logger.info(
        f"Replicating {file_hash} to {len(to_attempt)} new peers. Current copies: {len(current_locations)}, Target: {REPLICATION_FACTOR}"
    )
